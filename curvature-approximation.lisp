...
(defun fair-by-hessian (surface resolution target)
  (let* ((net (control-net surface))
	 ...
	 (x (make-array (list (* 12 (- ures 2) (- vres 2)) (* 3 (1- n) (1- m))) :initial-element 0.0d0))
			...
			(iter (for j from 0 to m)
				...
				(iter (for k from 0 below 2)
				      (iter (for l from 0 below 2)
						...
						(iter (with p = (v* (aref net i j) h))
						      (for c from 0 below 3)
						      (decf (aref y (+ row c) 0) (elt p c)))
					      (iter (for c from 0 below 3)
      ...
      (iter (with net = (control-net result))
	    ...
	    (iter (for j from 0 below (1- m))
		  (setf (aref net (1+ i) (1+ j))
			(list (elt solution (+ (* 3 (+ (* i (1- m)) j)) 0))
			      (elt solution (+ (* 3 (+ (* i (1- m)) j)) 1))
			      (elt solution (+ (* 3 (+ (* i (1- m)) j)) 2))))))
      result)))

(defmacro iter-surface (surface resolution (u v) &body body)
  (cl-utilities:with-gensyms (ui vi low high length sf re res)
    `(iter (with ,sf = ,surface)
	   (with ,re = ,resolution)
	   (with ,res = (if (listp ,re) ,re (list ,re ,re)))
	   (with ,low = (bss-lower-parameter ,sf))
	   (with ,high = (bss-upper-parameter ,sf))
	   (with ,length = (v- ,high ,low))
	   (for ,ui from 0 below (first ,res))
	   (for ,u = (+ (first ,low) (/ (* ,ui (first ,length)) (1- (first ,res)))))
	   (iter (for ,vi from 0 below (second ,res))
		 (for ,v = (+ (second ,low) (/ (* ,vi (second ,length)) (1- (second ,res)))))
		 ,@body))))
