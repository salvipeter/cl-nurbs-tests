    ...
    (if (eq dir 's)
        ...
        (or (handle-point-on-sides segments p)
            (dlet* (((p-1 p0 p1 p2) (segments-prev points segments))
                    ...
                    (a (in-system base-x base-y (v- p0 p1)))
                    (b (in-system base-x base-y (v- (affine-combine p0 1/3 p-1) p1)))
                    (c (in-system base-x base-y (v- (affine-combine p3 1/3 p4) p1)))
                    (d (in-system base-x base-y (v- p3 p1)))
                    (local-p (in-system base-x base-y (v- p p1)))
                    (((ax ay) (bx by) (cx cy) (dx dy) (x y)) (list a b c d local-p))
                    (s (fourth-degree-solver
                        (+ (* -1 ay l) (*  3 by l) (* -3 cy l) (*    dy l))
                        (+ (* -1 ax y) (*  3 bx y) (* -3 cx y) (*    dx y)
                           (*    ay x) (* -3 by x) (*  3 cy x) (* -1 dy x)
                           (*  3 ay l) (* -6 by l) (*  3 cy l))
                        (+ (*  3 ax y) (* -6 bx y) (*  3 cx y)
                           (* -3 ay x) (*  6 by x) (* -3 cy x)
                           (* -3 ay l) (*  3 by l))
                        (+ (* -3 ax y) (*  3 bx y) (* -1 l y)
                           (*  3 ay x) (* -3 by x) (* ay l))
                        (- (* ax y) (* ay x))
                        :min 0 :max 1))
                    (1-s (- 1 s)))
              (/ y (+ (* ay 1-s 1-s 1-s) (* by 3 1-s 1-s s) (* cy 3 1-s s s) (* dy s s s))))))))
    ...
    (if (eq dir 's)
        ...
        (or (handle-point-on-sides segments p)
            (dlet* (((p-1 p0 p1 p2) (segments-prev points segments))
                    ...
                    (X11 (* hs (+ (* (- a2x a1x) 1-2s) (* (- a3x a4x) 2s))))
                    (X12 (* 1-hs (+ (* (- b2x b1x) 2s) (* (- b3x b4x) 1-2s))))
                    (Y1 (+ (* hs (+ (* a1x 1-2s) (* a4x 2s)))
                           (* 1-hs (+ (* b1x 2s) (* b4x 1-2s)))
                           (* -1 x)))
                    (X21 (* hs (+ (* (- a2y a1y) 1-2s) (* (- a3y a4y) 2s))))
                    (X22 (* 1-hs (+ (* (- b2y b1y) 2s) (* (- b3y b4y) 1-2s))))
                    (Y2 (+ (* hs (+ (* a1y 1-2s) (* a4y 2s)))
                           (* 1-hs (+ (* b1y 2s) (* b4y 1-2s)))
                           (* -1 y)))
