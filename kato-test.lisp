...
(defmethod compute-distance ((type (eql 'perpendicular)) points segments p dir)
  (let ((p0 (elt segments 1))
	...
	(/ (point-line-distance p (list p0 p1))
	   (iter (for q in points)
                 (maximize (point-line-distance q (list p0 p1)))))
...
(defmethod compute-distance ((type (eql 'g2-sweep)) points segments p dir)
  ...
  (let* ((vectors (mapcar (lambda (x) (v- p x)) points))
    ...
    (labels ((inc (i) (mod (1+ i) n))
      ...
      (let* ((w (iter (for i from 0 below n)
                      ...
                      (when (< (abs ri) *epsilon*)
                        (return-from mean-value (elt values i)))
                      (when (and (< (abs Ai) *epsilon*)
                                 (< Di (- *epsilon*)))
                        (return-from mean-value
                          (/ (+ (* (elt values i) ri+1)
                                (* (elt values (inc i)) ri))
                             (+ ri ri+1))))
...
(defmethod compute-distance ((type (eql 'mean-kato)) points segments p dir)
  (if (eq dir 'd)
      (mean-distance points segments p)
      (let ((dprev (mean-distance points (segments-prev points segments) p))
            (dnext (mean-distance points (segments-next points segments) p)))
        (let ((dd (+ dprev dnext)))
          (if (< dd *epsilon*)
              0.5
              (/ dprev dd))))))
...
(defun quad-wachspress-distances (points)
  (let ((*wachspressp* t)
    ...
    (iter (for i from 0 below n)
          ...
          (for segments = (iter (for j from -2 below 2)
                                (collect (elt points (mod (+ i j) n)))))
          (for dc = (mean-distance points segments center))
          (collect (/ (- 1/2 dc) (* 4 dc (- 1 dc)))))))
...
(let* ((points (points-from-angles '(40 20 60 100 80)))
       (*wachspressp* t))
  (vectorized-distance-function-test
   points '(nil sd nil nil nil) "/tmp/proba2.ps"
