...
(defun blend-patch-meshes (a b)
  (let* ((res (array-dimensions a))
	 (midu (/ (first res) 2))
	 (midv (/ (second res) 2))
	 (result (make-array res)))
    (dotimes (i (first res))
	...
	(let* ((ad (expt (- (abs (- midu i)) midu) 2))
	       ...
	       (ratio (if (= (+ ad bd) 0) 0.5 (/ ad (+ ad bd)))))
	  (setf (aref result i j)
		(affine-combine (aref a i j) ratio (aref b i j))))))
    result))
...
border. The u border is at v=0 if VENDP is false; similarly for the v border."
  (let* ((nu (array-dimension up 0))
	 ...
	 (res-u (make-array (list nu nv)))
	 (res-v (make-array (list nu nv))))
    (dotimes (i nu)
      (dotimes (j nv)
	(flet ((get-dvec (array udir endp)
	  ...
	  (let ((du (get-dvec up nil vendp))
	    ...
	    (setf (aref res-u i j)
		  (v+ (first du) (v* (second du) (if vendp j (- nv j 1))))
		  (aref res-v i j)
		  (v+ (first dv) (v* (second dv) (if uendp i (- nu i 1)))))))))
    (list (blend-patch-meshes res-u res-v))))
