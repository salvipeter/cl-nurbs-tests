...
(defun write-bezier-ribbon-control-points (ribbons filename &key center concave)
  ...
  (let ((n (1- (length (first (first ribbons)))))
    ...
    (with-open-file (s filename :direction :output :if-exists :supersede)
      ...
      (iter (for i from 1 to (length concave))
            (format s "p ~a~%" (+ (* (length ribbons) (1+ n) (1+ m)) 1 i))))))

(defun normalize-smaller-angles (angles)
  (flet ((flip (a) (if (< a 0) (- (+ pi a)) (- pi a))))
    (let ((target (* (- (length angles) 2 (* 2 (count 0 angles :test #'>))) pi))
          (sum (reduce #'+ (mapcar (lambda (x) (flip x)) angles))))
      (when (and (/= target 0) (> (abs sum) *epsilon*))
        (let ((angle-multiplier (/ target sum)))
          (mapcar (lambda (x) (flip (* (flip x) angle-multiplier))) angles))))))

(defun normalize-inner-angles (angles)
  (flet ((flip (a) (- pi a)))
...
(defun normalize-domain (points)
  (let* ((min (reduce (lambda (a b) (mapcar #'min a b)) points))
         ...
         (center (affine-combine min 1/2 max)))
    (mapcar (lambda (p) (v* (v- p center) (/ 2 len))) points)))

(defun generate-angle-length-domain (angles lengths)
  (let* ((length-sum (reduce #'+ lengths))
...
(defun enlarge-domain-angles (angles &optional (factor 1.1))
  (flet ((flip (a) (- pi a)))
    (let* ((target (* (- (length angles) 2) pi))
           (sum (reduce #'+ (mapcar (lambda (x) (* (flip x) (if (< x 0) 1 factor))) angles)))
           (delta (/ (- target sum) (count 0 angles :test #'>))))
      (mapcar (lambda (x)
                (if (< x 0)
                    (flip (+ (flip x) delta))
                    (flip (* (flip x) factor))))
...
(defun segment-segment-distance (s1 s2)
  (let* ((p1 (first s1))
         (p2 (first s2))
         (v1 (v- (second s1) (first s1)))
         (v2 (v- (second s2) (first s2)))
         (d (v- p1 p2)))
    (macrolet ((x (var) `(first ,var))
               (y (var) `(second ,var)))
      (flet ((normal (v) (vnormalize (list (- (y v)) (x v)))))
        (let ((denom (- (* (x v2) (y v1))
                        (* (y v2) (x v1)))))
          (if (< (abs denom) *epsilon*)
              ;; Parallel
              ...
              (let* ((t1 (/ (- (* (y v2) (x d))
                     ...
                     (t2 (if (> (abs (x v2)) (abs (y v2)))
                             ...
                             (/ (+ (* t1 (y v1))
                                   (y d))
                                (y v2)))))
                (if (and (< 0 t1 1) (< 0 t2 1))
                    0
                    (let ((u1 (/ (scalar-product d v1) (- (vlength2 v1))))
                          (u2 (/ (scalar-product d v2) (vlength2 v2)))
                          (w1 (/ (scalar-product (v- d v2) v1) (- (vlength2 v1))))
                          (w2 (/ (scalar-product (v+ d v1) v2) (vlength2 v2))))
                      ;; try to match an endpoint and an interval
                      (cond ((and (< 0 u1 1) (<= t2 0))
                             (abs (scalar-product (normal v1) d)))
                            ((and (< 0 w1 1) (>= t2 1))
                             (abs (scalar-product (normal v1) (v- d v2))))
                            ((and (< 0 u2 1) (<= t1 0))
                             (abs (scalar-product (normal v2) d)))
                            ((and (< 0 w2 1) (>= t1 1))
                             (abs (scalar-product (normal v2) (v+ d v1))))
                            ;; endpoints
                            ((and (<= u1 0) (<= u2 0))
                             (vlength d))
                            ((and (<= w1 0) (>= u2 1))
                            ...
                            ((and (>= w1 1) (>= w2 1))
                             (vlength (v+ d (v- v1 v2))))
                            (t (warn "Invalid state")
                               (dprint s1 s2 t1 t2 u1 u2 w1 w2)
                               0)))))))))))

(defun segment-segment-distance-test (n resolution tolerance parallelp)
  (labels ((random-data ()
             (let* ((p1 (list (random 100.0) (random 100.0)))
                    (v1 (v- (list (random 100.0) (random 100.0)) '(50 50)))
                    (p2 (list (random 100.0) (random 100.0)))
                    (v2 (if parallelp
                            (v* (vnormalize v1) (- (random 100.0) 50))
                            (v- (list (random 100.0) (random 100.0)) '(50 50)))))
               (list p1 v1 p2 v2)))
           (approx-dist (p1 v1 p2 v2)
             (iter (for i from 0 below resolution)
                   (for t1 = (/ i (1- resolution)))
                   (for q1 = (v+ p1 (v* v1 t1)))
                   (minimizing
                    (iter (for j from 0 below resolution)
                          (for t2 = (/ j (1- resolution)))
                          (minimizing
                           (point-distance q1 (v+ p2 (v* v2 t2)))))))))
    (iter (for i from 1 to n)
          (format t "Test ~a...~%" i)
          (for (p1 v1 p2 v2) = (random-data))
          (for approx = (approx-dist p1 v1 p2 v2))
          (for guess = (segment-segment-distance (list p1 (v+ p1 v1))
                                                 (list p2 (v+ p2 v2))))
          (for err = (abs (- approx guess)))
          (when (> err tolerance)
            (warn "Error: ~a" err)
            (format t "~a~%" (list (list p1 (v+ p1 v1)) (list p2 (v+ p2 v2))))))))
...
(defun domain-valid-p (points &optional (minimum-distance 0.2))
  (let ((n (length points))
    ...
    (iter (for i from 0 below (- n 2))
          (iter (for j from (+ i 2) below n)
                (when (and (or (> i 0) (< j (1- n)))
                           (< (segment-segment-distance (elt lines i) (elt lines j))
                              minimum-distance))
...
(defun concave-generalized-bezier-eval (map points p degree ribbons center &key concave (use-d t))
  ...
  (let ((n (length points))
    ...
    (iter (with l = (harmonic-coordinates map points p))
          (for (cp i j) in concave)
          (for di = (barycentric-d l i))
          (for dj = (barycentric-d l j))
          (for weight = (* (bernstein 3 2 di) (bernstein 3 2 dj)))
          (setf wsum (+ wsum weight)
                result (v+ result (v* cp weight))))
    #+nil(let ((center-blend (gb-center-blend map points p)))
...
(defun generate-twist-cp (ribbons i)
  (let* ((n (length ribbons))
         (i-1 (mod (1- i) n)))
    (macrolet ((p (i j k) `(elt (elt (elt ribbons ,i) ,k) ,j)))
      (affine-combine
       (v+ (p i-1 3 1) (v- (p i-1 3 1) (p i-1 2 1)))
       1/2
       (v+ (p i 0 1) (v- (p i 0 1) (p i 1 1)))))))

(defun concave-generalized-bezier-test (input-file concave output-file &key cgb)
  "CONCAVE is a list of concave vertex indices."
  (let ((ribbons (if cgb (read-cgb-ribbons input-file) (load-ribbons input-file))))
    (unless cgb
      (dolist (i concave)
        (mirror-concave-corner ribbons i)))
    (let* ((center (if cgb (second ribbons) (generalized-bezier-generate-center ribbons)))
           (ribbons (if cgb (first ribbons) ribbons))
           (domain (domain-from-ribbons-angular-concave ribbons))
           (concave-cps (mapcar (lambda (i)
                                  (list (generate-twist-cp ribbons i)
                                        i (mod (1- i) (length ribbons))))
                                concave)))
      (write-bezier-ribbon-control-points ribbons "/tmp/ribbons.obj"
                                          :center center :concave concave-cps)
      ...
      (harmonic:with-harmonic-coordinates (hmap domain :levels 10)
        (destructuring-bind (vertices triangles)
            (shewchuk-triangle:mesh domain *resolution*)
          (flet ((foo (p)
                   (concave-generalized-bezier-eval hmap domain p 3 ribbons center
                                                    :concave concave-cps)))
...
(defun convert-gbp-to-cgb (model)
  (let* ((tests "/Shares/GrafGeo/Polar/bezier-ribbon/")
         (name (first model))
         (concave (rest model))
         (path (format nil "~a~a~a" *dropbox* tests name))
         (gbp (format nil "~a~a" path ".gbp"))
         (ribbons (load-ribbons gbp)))
    (dolist (i concave)
      (mirror-concave-corner ribbons i))
    (let ((domain (domain-from-ribbons-angular-concave ribbons)))
      (multiple-value-bind (rib center)
          (read-gbp gbp)
        (declare (ignore rib))
        (write-domain-ribbons domain '() (format nil "~a~a" path ".ps"))
        (external-program:run "/usr/bin/ps2pdf"
                              (list (format nil "~a~a" path ".ps")
                                    (format nil "~a~a" path ".pdf")))
            ...
            (dotimes (i n)
              (dotimes (j m)
                (if (or (zerop i) (zerop j))
                    (format s "255 0 0 setrgbcolor~%2 setlinewidth~%")
                    (format s "0 0 0 setrgbcolor~%1 setlinewidth~%"))
                (unless (zerop i)
                  (format s "newpath~%~{~f ~}moveto~%~{~f ~}lineto~%stroke~%"
                          (scale (elt (elt ribbon j) i))
                          (scale (elt (elt ribbon j) (1- i)))))
                (if (or (zerop i) (= i (1- n)) (zerop j))
                    (format s "255 0 0 setrgbcolor~%2 setlinewidth~%")
                    (format s "0 0 0 setrgbcolor~%1 setlinewidth~%"))
                (unless (zerop j)
                  (format s "newpath~%~{~f ~}moveto~%~{~f ~}lineto~%stroke~%"
...
(let* ((tests "/Shares/GrafGeo/Polar/bezier-ribbon/")
       ...
       ;; (gbp (format nil "~a~a~a" *dropbox* tests "ConcaveTest_Plane.gbp"))    ; 4 5
       ;; (gbp (format nil "~a~a~a" *dropbox* tests "ConcaveTest_Cylinder.gbp")) ; 4 5
       ;; (gbp (format nil "~a~a~a" *dropbox* tests "ConcaveFromSquare_V31.gbp")); 1 4
       (cgb (format nil "~a~a~a" *dropbox* tests "nadrag.cgb")); 1 5 6 10
       (*resolution* 0.0001))
  ;; (concave-generalized-bezier-test gbp '(1 4) "/tmp/felulet.obj")
  (concave-generalized-bezier-test cgb '(1 5 6 10) "/tmp/felulet.obj" :cgb t)
  )
...
(let ((points (normalize-domain '((0 0) (6 0) (6 6) (4 6) (4 4) (2 4) (2 6) (0 6))))
  ...
  (flet ((generate (sh i)
           (flet ((fn (p)
                    (let ((l (maximum-entropy-coordinates points p)))
                      (if (eq sh 's)
                          (barycentric-s l i)
                          (barycentric-d l i)))))
