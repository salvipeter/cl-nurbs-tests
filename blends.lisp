  ...
  (let ((n *exponent*)
    ...
    (case derivative
      ...
      (1 (/ (- (+ (* n (expt d1 (1- n)) (+ (expt d0 n) (expt d1 n)))
		  (* n (expt d1 n) (- (expt d0 (1- n)) (expt d1 (1- n))))))
	    (expt (+ (expt d0 n) (expt d1 n)) 2)))
      (2 (/ (- (* (- (* n (1- n) (expt d1 (- n 2)) (+ (expt d0 n) (expt d1 n)))
		     (* n (1- n) (expt d1 n) (+ (expt d0 (- n 2)) (expt d1 (- n 2)))))
		  (expt (+ (expt d0 n) (expt d1 n)) 2))
...
(defun tomi-hybrid (p0 t0 p1 t1 u &key (derivative 0))
  (let ((d0 u)
	(d1 (- 1 u)))
    (case derivative
      (0 (v+ (v* (v+ p0 (v* t0 *ribbon-multiplier-start* d0))
		 (hermite-blend-function 'point 'start d0))
	     (v* (v+ p1 (v* t1 *ribbon-multiplier-end* d1))
		 (hermite-blend-function 'point 'start d1))))
      (1 (v+ (v* t0 *ribbon-multiplier-start* (hermite-blend-function 'point 'start d0))
	     (v* (v+ p0 (v* t0 *ribbon-multiplier-start* d0))
		 (hermite-blend-function 'point 'start d0 :derivative 1))
	     (v* t1 *ribbon-multiplier-end* -1 (hermite-blend-function 'point 'start d1))
	     (v* (v+ p1 (v* t1 *ribbon-multiplier-end* d1))
		 -1 (hermite-blend-function 'point 'start d1 :derivative 1))))
      (2 (v+ (v* t0 2 *ribbon-multiplier-start*
		 (hermite-blend-function 'point 'start d0 :derivative 1))
	     (v* (v+ p0 (v* t0 *ribbon-multiplier-start* d0))
		 (hermite-blend-function 'point 'start d0 :derivative 2))
	     (v* t1 2 *ribbon-multiplier-end*
		 (hermite-blend-function 'point 'start d1 :derivative 1))
	     (v* (v+ p1 (v* t1 *ribbon-multiplier-end* d1))
		 (hermite-blend-function 'point 'start d1 :derivative 2)))))))

(defun write-tomi-hybrid (p0 t0 p1 t1 filename)
  (let (curve curvature derivative blend blend-derivative)
    (iter (for i from 0 below *resolution*)
	  (for u = (/ i (1- *resolution*)))
	  (with curve-fn = (lambda (x d) (tomi-hybrid p0 t0 p1 t1 x :derivative d)))
	  (for p = (funcall curve-fn u 0))
	  (collect p into tcurve)
	  (when (= (mod i 10) 5)
	    (let ((d (funcall curve-fn u 1)))
	      (collect (list p (v+ p (v* (vnormalize (list (- (second d)) (first d)))
		       ...
		       into tcurvature)))
	  (collect (funcall curve-fn u 1) into tderivative)
	  (collect (list u (hermite-blend-function 'point 'start u)) into tblend)
	  (collect (list u (hermite-blend-function 'point 'start u :derivative 1))
		   into tblend-derivative)
	  (finally (setf curve tcurve curvature tcurvature derivative tderivative
			 blend tblend blend-derivative tblend-derivative)))
...
#+nil
(let ((*resolution* 200))
  (write-tomi-hybrid '(0.3 0.3) '(0.1 0.7) '(0.8 0.3) '(-0.1 0.8) "/tmp/proba3.ps"))
...
(defun five-point-derivative (curve-fn u &key (h 1.0d-4) (derivative 1))
  (v* (v+ (v* (funcall curve-fn (+ u (* 2 h)) (1- derivative)) -1)
	  (v* (funcall curve-fn (+ u h)       (1- derivative))  8)
	  (v* (funcall curve-fn (- u h)       (1- derivative)) -8)
	  (v* (funcall curve-fn (- u (* 2 h)) (1- derivative))  1))
      (/ (* 12 h))))
...
(let ((p0 '(0.3 0.3))
      ...
      (t1 '(-0.1 0.8))
      (resolution 100))
  (flet ((hermite-fn (x d) (hermite p0 t0 p1 t1 x :derivative d))
	 (tomi-fn (x d) (tomi p0 t0 p1 t1 x :derivative d))
	 (hybrid-fn (x d) (tomi-hybrid p0 t0 p1 t1 x :derivative d)))
    (iter (for u from 0.0d0 to 1.0d0 by (/ (1- resolution)))
	  (sum (point-distance (five-point-derivative #'hermite-fn u)
			       (hermite-fn u 1))
	       into hermite-1-error)
	  (sum (point-distance (five-point-derivative #'hermite-fn u :derivative 2)
			       (hermite-fn u 2))
	       into hermite-2-error)
	  (sum (point-distance (five-point-derivative #'tomi-fn u)
			       (tomi-fn u 1))
	       into tomi-1-error)
	  (sum (point-distance (five-point-derivative #'tomi-fn u :derivative 2)
			       (tomi-fn u 2))
	       into tomi-2-error)
	  (sum (point-distance (five-point-derivative #'hybrid-fn u)
			       (hybrid-fn u 1))
	       into hybrid-1-error)
	  (sum (point-distance (five-point-derivative #'hybrid-fn u :derivative 2)
			       (hybrid-fn u 2))
	       into hybrid-2-error)
	  (finally (format t "Hermite 1st derivative error: ~f~%" (/ hermite-1-error resolution))
		   (format t "Hermite 2st derivative error: ~f~%" (/ hermite-2-error resolution))
		   (format t "Hybrid 1st derivative error: ~f~%" (/ hybrid-1-error resolution))
		   (format t "Hybrid 2st derivative error: ~f~%" (/ hybrid-2-error resolution))
		   (format t "Tomi 1st derivative error: ~f~%" (/ tomi-1-error resolution))
		   (format t "Tomi 2st derivative error: ~f~%" (/ tomi-2-error resolution))))))
