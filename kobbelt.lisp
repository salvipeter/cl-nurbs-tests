...
(defun approximate-normal (obj i)
  (let* ((p (elt (points obj) i))
	 ...
	 (neighborhood (point-neighbors p))
	 (x (make-array (list (1+ (* 3 (length neighborhood))) 3)
			:initial-element 0.0d0))
	 (y (make-array (list (1+ (* 3 (length neighborhood))) 1)
			:initial-element 0.0d0)))
    (setf (aref x 0 0) 1.0d0 (aref x 0 1) 1.0d0 (aref x 0 2) 1.0d0
	  (aref y 0 0) 1.0d0)
    (iter (for i upfrom 0)
	  (for neighbor in neighborhood)
	  (for d = (v- (neighbor-coordinates obj neighbor) pos))
	  (iter (for j from 0 below 3)
		(setf (aref x (+ (* 3 i) j 1) j) (elt d j))))
    (vnormalize (coerce (lu-solver:least-squares x y) 'list))))
...
(defun parameterize-projection (obj i)
  ...
  (let* ((p (elt (points obj) i))
	 ...
	 (one-point (neighbor-coordinates obj (first (point-neighbors p))))
	 (diff (v- one-point pos))
	 (u0 (vnormalize (v- diff (v* n (scalar-product diff n)))))
	 (v0 (vnormalize (cross-product u0 n))))
    (iter (for neighbor in (point-neighbors p))
	  (for coordinates = (neighbor-coordinates obj neighbor))
